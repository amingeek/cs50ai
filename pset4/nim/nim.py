...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        # convert state to tuple, just in case 'state' is a list
        state = tuple(state)

        # if given (state, action) key is present in the Q-value dictionary
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        """
        # update the Q-value dictionary according to the formula
        state = tuple(state)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if epsilon == False:
            ...
            prob = [1 - self.epsilon, self.epsilon]
        
        rest = list(Nim.available_actions(state))
        max_value = -1
        best_move = rest[0]
        ...
        rest.remove(best_move)

        options = ['best', 'random']
        choice = random.choices(options, weights=prob)
        if choice[0] == 'best':
            return best_move
        else:
            if len(rest) == 0:
                return best_move
            choice = random.choices(rest)
            return choice[0]
